...
extension BIP39.DerivationAlgorithm.HashingFunction {
...
extension Collection where Element == BIP39.DerivationAlgorithm {
    func seed(from words: [String]) throws -> [UInt8] {
        ...
        try forEach({
            ...
            case let .pkcs5(salt, password, iterations, klength):
                value = try PKCS5.PBKDF2<SHA512>().calculate(
                    password: value,
...
public extension Array where Element == BIP39.DerivationAlgorithm {
    ...
    static func ton(iterations: Int = 100_000, klength: Int = 32) -> [Element] {
        return [
            .hmac(kind: .sha512),
            .pkcs5(
